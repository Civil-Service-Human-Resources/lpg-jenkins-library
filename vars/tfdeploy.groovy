def call(def terraformModuleName, def terraformVarTag, def environment) {
    ...
    dir("lpg-terraform-paas/environments/master") {
        ...
        sh "terraform apply -target=module.${terraformModuleName} -var '$terraformVarTag=${env.BRANCH_NAME}-${env.BUILD_ID}' -var 'docker_registry_server_username=${acr_username}' -var 'docker_registry_server_password=${acr_password}' -lock-timeout=120s -auto-approve"
    }
}